...
- **49 tool** — `dnspy_info`; **phân tích tĩnh** (`decompile` C#/IL, `list_types`, `list_methods`, `search`, `find_references` method/field/type, `find_implementations`, `type_hierarchy`, `call_graph`, `extract_iocs`, `list_resources`/`extract_resource`); `decrypt_strings` (giải mã chuỗi động); `dump_module` (dump module packed từ RAM: section-table sizing, unmap, dựng lại COR20 anti-dump); `dbg_run_to`; `bp_add_trace`/`trace_log` (tracepoint log-and-continue); và điều khiển phiên/breakpoint/step/inspect
