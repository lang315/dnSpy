...
| `call_graph` | `module` (bắt buộc), `method`\|`token`, `direction`, `depth`, `max_nodes` | Đồ thị caller/callee của 1 method tới độ sâu cho trước (điều hướng luồng trong module). `direction`=callers\|callees\|both; `depth` mặc định 2. |
| `diff_assemblies` | `old`, `new` (bắt buộc), `include_methods`, `max` | So 2 module: type/method **thêm/bớt** (theo tên + chữ ký, không theo token) — so phiên bản. |
...
| `heap_stats` | `top` | Histogram object **sống** theo type (count + tổng byte) — tổng quan size/leak. Cần process paused. |
