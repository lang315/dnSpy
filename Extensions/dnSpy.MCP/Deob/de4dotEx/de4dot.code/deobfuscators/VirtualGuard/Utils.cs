...
namespace de4dot.code.deobfuscators.VirtualGuard
{
    public static class Utils
    {
        ...
        public static void RemoveCall(MethodDef callingMethod, MethodDef toRemove)
        {
            foreach (var item in callingMethod.Body.Instructions)
            {
                if (item.OpCode == OpCodes.Call && item.Operand == toRemove)
