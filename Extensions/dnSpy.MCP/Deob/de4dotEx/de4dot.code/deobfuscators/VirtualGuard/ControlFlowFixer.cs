...
namespace de4dot.code.deobfuscators.VirtualGuard
{
    internal class ControlFlowFixer : IBlocksDeobfuscator
    {
        ...
        private void ProcessBlock(List<Block> switchCaseBlocks, Block block, Block switchBlock)
        {
            ...
            var targetBlock = targets[switchCaseIndex.Value];
            Logger.v(targetBlock.FirstInstr.ToString());
            targetBlock.SwitchData.Key = key;

            var inss = block.Instructions;
            var inssCount = inss.Count;

            int newArrIdx = -1;

            for (int i = inssCount - 1; i >= 0; i--)
            {
                if (inss[i].OpCode == OpCodes.Newarr)
                {
                    newArrIdx = i;
                    break;
                }
            }
            if (inssCount > 9 && newArrIdx != -1)
            {
                int nopBeginIdx;
                Local v1 = (Local)inss[inssCount - 8].Operand;
                if (inss[newArrIdx + 1].Operand == v1)
                {
                    nopBeginIdx = newArrIdx - 1;
                    block.Remove(nopBeginIdx, inssCount - nopBeginIdx);
                }
            }
            else
            {
        ...
        {
            //Logger.v("MethodBlocks Count : " + methodBlocks.Count);
        ...
        private int? GetSwitchKey()
        {
            ...
            if (!val.IsInt32())
                return null;
            if (!(val is Int32Value value) || !value.AllBitsValid())
                return null;
            return value.Value;
        }
