...
### dnSpy.MCP extension
`Extensions/dnSpy.MCP` embeds an MCP (Model Context Protocol) server inside the dnSpy process and exposes the debugger to AI agents over loopback HTTP (`127.0.0.1:27115/mcp`, JSON-RPC 2.0). 54 tools: static analysis (decompile to C#/IL incl. batch decompile to a folder, list/search types & methods, find references to methods/fields/types & implementations, type hierarchy, call graph, assembly diff, IOC & resource extraction — no debug session needed), GC-heap inspection of a paused process (histogram / find instances of a type / inspect an object, via a bundled out-of-process ClrMD helper so modern .NET regions heaps work), session control, breakpoints (by IL token, method name or source line; run-to; log-and-continue tracepoints), stepping, inspection of a paused process (threads, call stack, locals, eval incl. func-eval, memory r/w), dynamic string decryption (func-eval a decryptor at its call sites), and dumping a packed/protected module from live process memory (section-table sizing, memory→file unmap, anti-dump COR20 reconstruction). Selected read tools also return MCP structured content (outputSchema + structuredContent, spec 2025-06-18). Read `Extensions/dnSpy.MCP/README.md` and `USAGE.md` before touching it. Load-bearing facts:
