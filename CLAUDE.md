...
### dnSpy.MCP extension
`Extensions/dnSpy.MCP` embeds an MCP (Model Context Protocol) server inside the dnSpy process and exposes the debugger to AI agents over loopback HTTP (`127.0.0.1:27115/mcp`, JSON-RPC 2.0). 33 tools: session control, breakpoints (by IL token, method name or source line), stepping, and inspection of a paused process (threads, call stack, locals, eval incl. func-eval, memory r/w). Read `Extensions/dnSpy.MCP/README.md` and `USAGE.md` before touching it. Load-bearing facts:
